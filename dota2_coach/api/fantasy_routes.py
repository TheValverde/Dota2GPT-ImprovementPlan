...
def roster(
    ...
    try:
        return tracker.summary(_span(amount, unit))
...
def add_player(
    ...
    try:
        return tracker.add_player(body.account_id, _span(body.amount, body.unit))
    except CoachError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
...
def player_card(
    ...
    try:
        return tracker.player_card(account_id, _span(amount, unit))
    except TrackedPlayerNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except CoachError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
...
def refresh(
    ...
    try:
        return tracker.refresh(_span(body.amount, body.unit), body.account_id)
    except TrackedPlayerNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except CoachError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
