...
**Coach.** `GET /matches/{match_id}` plus constants. The brief is an event ledger (kills, towers, Roshan, couriers, focus wards with map regions, runes, item completions, all on one clock) plus computed context: objective windows (the kills around every tower and Roshan, with a flag when the focus team lost a building while the focus player fought elsewhere), death windows (killer, fight, enemy gold swing, delayed item for every focus death), gold swings, phase summaries (laning, midgame, closing), a lane swing vs the lane opponent (gold/XP/CS gaps after early kills, that opponent's first kill and items), lane matchup (safelane vs enemy offlane using parsed `lane`), inferred assignment with a role rubric, curated ability facts for the ten heroes in the lobby, recent form from `GET /players/{id}/matches`, and OpenDota benchmarks. After the LLM writes the report, a coverage check verifies every focus death, lost objective, Roshan, and gold swing appears with its timestamp; if any are missing, one repair call asks the model to fix the report. Percentiles are a hero-wide curve, not a medal bracket. If `version` is missing, the overlay can `POST /request/{match_id}`, poll `GET /request/{jobId}`, then re-analyze. **Coach latest parsed** scans `GET /players/{id}/recentMatches` for a row with `version`, then `GET /players/{id}/matches?project=version` over the last 50 games if recent games are all unparsed.
